...
def make_atari(env_id, max_episode_steps=None, sticky_actions: bool = False):
    """
    Args:
        env_id: use underscore for games with more than one word
            e.g. pong, enduro, space_invader
    """
    env_id = env_id.lower()
    assert (
        env_id in ALL_ATARI_GAMES
    ), f'invalid Atari game ID "{env_id}". \nSelect one of {ALL_ATARI_GAMES}'
    env_id = "".join([g.capitalize() for g in env_id.split("_")])
    version = 0 if sticky_actions else 4
    env_id = f"{env_id}NoFrameskip-v{version}"
    env = gym.make(env_id)
...
ALL_ATARI_GAMES = [
    ...
    'robotank', 'seaquest', 'skiing', 'solaris', 'space_invaders', 'star_gunner',
